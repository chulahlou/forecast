...
class Forecast(dict):
    ...
    def build_estimator(self, epochs: int = 10, learning_rate: float = 1e-3, num_batches_per_epoch: int = 100, context_length: int = 2 * 7 * 24):
    ...
    def eval(self):
        forecast_it, ts_it = make_evaluation_predictions(dataset=self.dataset.test, predictor=self.predictor)
        forecasts = [f for f in forecast_it]
        tss = list(ts_it)
        evaluator = Evaluator(quantiles=[0.1, 0.5, 0.9])
        metrics, _ = evaluator(iter(tss), iter(forecasts), num_series=len(self.dataset.test))
        return forecasts, tss, pd.DataFrame.from_records(metrics, index=["DeepAR"]).transpose()

    def get_dataset(self, name: str = "electricity"):
        if name not in dataset_recipes.keys():
            raise KeyError(f"The dataset name {name} is not supported. \nPlease select from " +
